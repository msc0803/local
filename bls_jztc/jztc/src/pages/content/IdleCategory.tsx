...
import { PlusOutlined, EditOutlined, DeleteOutlined, ReloadOutlined, LinkOutlined, PictureOutlined } from '@ant-design/icons';
import './styles.css';
import { 
  getIdleCategoryList, 
  createIdleCategory, 
...
const IdleCategory: React.FC = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      if (res.code === 0) {
        ...
        message.error(res.message || '获取闲置分类失败');
      }
    } catch (error) {
      message.error('获取闲置分类失败');
      console.error('获取闲置分类失败:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await deleteIdleCategory(id);
